...
export declare interface DMMFClass
	extends DMMFDatamodelHelper, DMMFMappingsHelper, DMMFSchemaHelper {
}
			...
			[K in keyof S as S[K] extends false | undefined | null ? never : K]:
				S[K] extends true ? P extends {
					...
					: P extends {
						...
						: never
					: K extends '_count' ? Count<GetFindResult<P, S[K]>>
					: never;
		}
		& (A extends {
			include: any;
		} & Record<string, unknown> ? P['scalars']
			: unknown)
	: P['scalars'];
...
> {
	...
	createBatch(
		messages: ProtocolMessage<EngineQueryType>[],
	): EngineBatchQueries;
}
...
declare type WrapPropsInFnDeep<T> =
	& {
		[K in keyof T]: T[K] extends Function ? T[K]
			: T[K] extends object ? WrapPropsInFnDeep<T[K]>
			: () => T[K];
	}
	& {};
